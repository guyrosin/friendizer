...
public class RequestInfo {
	...
	public static RequestInfo processRequest(HttpServletRequest req, HttpServletResponse resp, ServletContext ctx) throws IOException {

		User user = null;
		RequestInfo ri = new RequestInfo();
		ri.ctx = ctx;

		// Access user information using OAuth
		...
		if (user == null) {
			// OAuth didn't work -> try using Users API
		...
		ri.deviceRegistrationID = ri.getParameter(Util.DEVICE_REGISTRATION_ID);
	...
	private void initDevices(ServletContext ctx) {
		...
		try {
			devices = DeviceInfo.getDeviceInfoForUser(userName);
	...
	public void deleteRegistration(String regId) {
		...
		PersistenceManager pm = PMF.get().getPersistenceManager();
		try {
			List<DeviceInfo> registrations = DeviceInfo.getDeviceInfoForUser(userName);
