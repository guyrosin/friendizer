...
public final class ServerFacade {
	...
	public static UserInfo userDetails(long userID) throws Exception {
		...
		in.close();
		return new UserInfo(userID, value, money, owner);
	}
	...
	public static void buy(long userID, long buyID) throws Exception {
		URL url = new URL(serverAddress + "buy?userID=" + userID + "&buyID="
				+ buyID);
		BufferedReader in = new BufferedReader(new InputStreamReader(
				url.openStream()));
		in.close();
	...
	public static void changeLocation(long userID, double xCord, double yCord)
			throws Exception {
		URL url = new URL(serverAddress + "changeLocation?userID=" + userID
	...
	public static long[] nearbyUsers(long userID, double distance)
			throws Exception {
